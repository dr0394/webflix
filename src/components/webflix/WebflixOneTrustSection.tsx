...
interface WebflixOneTrustSectionProps {
...
const WebflixOneTrustSection: React.FC<WebflixOneTrustSectionProps> = ({
  trustPoints = [],
  reviews = [],
  googleRating = 5.0,
  googleReviewCount = 0,
  ...
  if (trustPoints.length === 0 && reviews.length === 0) {
    return null;
  }
              ...
              <div className="bg-slate-50 rounded-2xl p-6 text-center border border-slate-100">
                <div className="mb-6">
                  ...
                  <div className="flex items-center justify-center gap-4 mb-6">
                    <span className="text-4xl font-bold text-slate-800 font-poppins">{googleRating.toFixed(1)}</span>
                    <div className="flex gap-1">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} className="w-6 h-6 fill-yellow-400 text-yellow-400" />
                      ))}
                    </div>
                    <span className="text-slate-600 font-medium font-montserrat">({googleReviewCount})</span>
                  </div>
                </div>
...
export default WebflixOneTrustSection;
