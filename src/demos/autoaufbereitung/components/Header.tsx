...
import { Car, Star, Settings } from 'lucide-react';
...
const Header: React.FC<HeaderProps> = ({ minimal = false, darkBackground = false, customData = {}, isPreviewMode = false }) => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <a href="/" className="flex items-center space-x-2">
            <div data-editable="header_logo" className="text-2xl font-bold text-white font-poppins">
              {getContent('header_logo', 'Dein Logo')}
            </div>
          </a>
          ...
          {!minimal && (
            <nav className="hidden lg:flex items-center space-x-8">
              ...
              >
                <span data-editable="header_nav_1">{getContent('header_nav_1', 'Home')}</span>
              ...
              <a
                ...
                className="text-white/90 hover:text-white transition-all duration-300 font-medium relative group drop-shadow-md"
              >
                <span data-editable="header_nav_2">{getContent('header_nav_2', 'Konfigurator')}</span>
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-white transition-all duration-300 group-hover:w-full"></span>
              </a>
              <a 
                href="#contact" 
                className="text-white/90 hover:text-white transition-all duration-300 font-medium relative group drop-shadow-md"
              >
                <span data-editable="header_nav_3">{getContent('header_nav_3', 'Kontakt')}</span>
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-white transition-all duration-300 group-hover:w-full"></span>
              </a>
              ...
              >
                ...
                <span data-editable="header_cta">{getContent('header_cta', 'Jetzt anfragen')}</span>
              </a>
            </nav>
          )}
